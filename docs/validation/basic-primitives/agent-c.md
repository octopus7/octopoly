...
## 25-model catalog extension

The second approved catalog extension adds exactly 18 models after duplicate Sneaker/Kettle entries were removed: Cat, Dog, Fish, Turtle, Elephant, Cup, Chair, Flowerpot, Kettle, Sneaker, Backpack, Helmet, Gamepad, Camera, Bicycle Saddle, Car, Rocket, and Treasure Chest.

Each model has a deterministic code-native recipe, exact topology and landmark tests, a package-local entry method through the existing atomic creation path, and an accessible `Add …` button. The New Scene catalog remains native-keyboard navigable and is contained in a semantic scrollable group while preserving 44 CSS px minimum targets.

Actual Headless Chrome 145/WebGL2 ran all 25 built-ins through Add, state-verified Undo-to-empty, exact topology/stable-ID Redo restoration, reference-free Move, single-face Extrude with topology/history change, Save, Reload with exact edited fingerprints, and OBJ/GLB export. Every relevant phase has finite non-empty renderer evidence; all 25 passed with no warning, console, page, or JavaScript errors. Schema v3 records the live face-ID set and face-cycle sizes at each checkpoint, exact action/history states, recipe-authoritative creation counts, and parsed export metadata. OBJ payload validation parses finite vertex and triangular face records and compares them to the edited mesh triangulation. GLB validation checks magic/version/declared length, JSON and BIN chunks, accessors, triangle primitive, POSITION count, and index-derived triangle count against the parsed OBJ. The committed `catalog-runner.ts` generated the raw per-scenario SmokeResult records in `desktop-chrome-catalog-25.json` with candidate base `165508b5a489f9d39b6491531aa1356ceb6f2d0b` and explicit runner/harness/evaluator/schema provenance. Independent reviews `deleg_848b155f` and `deleg_d554e228` both failed earlier candidate diffs; their findings were corrected, but review status remains `PENDING_AFTER_FIX` and no replacement independent PASS is claimed.
