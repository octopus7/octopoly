...
## 범위 기반 비례 편집·새작업·정점 표시·카메라 불변성 | 시작: 2026-08-18 13:42:02 KST | 종료: 2026-08-18 14:22:45 KST | 소요: 41분

- ROADMAP `+3차`의 기본 OFF 비례 편집을 구현했다. 선택 정점 weight 1, Smooth/Linear/Sharp falloff, 조절 가능한 radius, `Connected only`를 제공하며 gizmo, view-plane, constrained-plane 이동을 한 번의 weighted workspace mutation·scene publication·autosave transaction으로 처리한다.
- `Connected only`는 단순 component 판정이 아니라 triangle edge 길이 기반 bounded Dijkstra로 model-space topology 경로 거리를 계산한다. 접힌 메시에서 공간상 가깝지만 topology 경로가 긴 정점은 radius 밖으로 제외한다.
- drag 시작 시 mesh ID, selected vertex, 전체 weight를 snapshot해 연속 이동 중 geometry 변화로 falloff가 drift하거나 selection 변경 후 남은 delta가 다른 정점으로 넘어가지 않게 했다. weighted autosave 실패는 workspace, revision, selection 및 scene publication을 원자적으로 보존한다.
- viewport-independent influence ring과 weighted point overlay를 추가했다. overlay는 pointer input을 가로채지 않으며 실제 이동 weight는 모두 유지하되 대형 mesh의 DOM/projection 표시는 최대 512개를 균등 sampling한다. File, movement, proportional popover는 상호 배타적으로 동작한다.
- File 메뉴 첫 command를 사용자 지정 문구와 순서인 `새작업`, `저장`, `불러오기`로 정리했다. `새작업`은 active Facial runtime과 pending project/texture/OBJ 작업을 종료하고 저장 workspace를 제거한 뒤 최초 기본 cube viewport로 복귀한다. cube staging 또는 storage 삭제 실패 시 저장 workspace를 유지한 새 Facial runtime으로 rollback한다.
- visible vertex handle pass에 bounded clip-space depth bias를 적용해 depth test를 유지하면서도 메시 표면에 가시성이 확정된 일반·선택 정점의 5/8 CSS-pixel square가 삼각면 depth에 부분적으로 잘리지 않고 온전하게 보이도록 했다. face와 edge pass의 bias는 0이며 기존 `LESS`, cull 및 program 상태 복원을 유지한다.
- wheel/pinch로 사용자가 정한 camera distance를 별도 snapshot state로 추적한다. 최초 frame 중에는 bounds fit을 유지하지만 이후 selection, 동일 scene publication, 빈 공간 orbit drag, geometry bounds 갱신 및 resize가 근접 zoom을 강제로 뒤로 밀지 않는다. camera transaction rollback은 manual-distance 상태까지 복원한다.
- `.octopoly` exact v1 schema는 변경하지 않았다. 비례 편집 결과 geometry는 기존 workspace로 저장되며 비례 UI 설정은 session tool state로 유지된다. 새작업은 runtime 전체를 종료하므로 비례 편집도 기본 OFF로 재시작한다.
