...
import { isLookdevRealtimeProgramWithinBudget } from "./budget";

export const LOOKDEV_REALTIME_VERTEX_SHADER = `#version 300 es
...
export const LOOKDEV_REALTIME_FRAGMENT_SHADER = `#version 300 es
precision highp float;
precision highp int;
...
void main() {
  ...
  vec3 environmentSpecular = fresnelSchlick(nDotV, f0) * environment * (1.0 - 0.5 * roughness);
  vec3 hdr = direct + (environmentDiffuse + environmentSpecular) * uEnvironmentIntensity + emissive;
  vec3 display = pow(acesToneMap(max(hdr * uExposure, vec3(0.0))), vec3(1.0 / 2.2));
  outColor = vec4(display, opacity * uBaseColor.a);
}`;

export const LOOKDEV_REALTIME_PROGRAM: ShadingProgramDescriptor = Object.freeze({
  language: "glsl-es-300",
  vertexShader: LOOKDEV_REALTIME_VERTEX_SHADER,
  fragmentShader: LOOKDEV_REALTIME_FRAGMENT_SHADER,
...
if (!isLookdevRealtimeProgramWithinBudget(LOOKDEV_REALTIME_PROGRAM)) {
  throw new Error("Realtime lookdev GLSL exceeds the frozen mobile shader budget");
}
